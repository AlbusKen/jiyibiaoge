...
export const defaultSettings = await switchLanguage('__defaultSettings__', {
  ...
  `,
    ...
    // 双步破限预设
    step_by_step_system_prompt: "You are a helpful assistant. Based on the provided table data and chat history, your task is to identify and list the necessary modifications to the tables. Output these modifications as a JSON array of action objects. Each object must have an 'action' (insert, update, or delete), 'tableIndex' (integer), and 'data' (for insert/update) or 'rowIndex' (for update/delete). For 'update' and 'insert', 'data' should be an object where keys are column indices (as strings, e.g., \"0\", \"1\") and values are the new cell content. Ensure row indices are accurate for updates and deletions. Focus only on outputting the JSON array of actions. Current table data: $0\n\nRecent chat history: $1",
    // 双步字数累加
