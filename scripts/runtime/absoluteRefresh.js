...
export async function executeIncrementalUpdateFromSummary(
    ...
    try {
        ...
        if (isStepByStepSummary) {
            systemPrompt = USER.tableBaseSetting.step_by_step_system_prompt || USER.tableBaseSetting.refresh_system_message_template;
            console.log("Using step-by-step system prompt.");
        } else {
            systemPrompt = USER.tableBaseSetting.refresh_system_message_template;
            console.log("Using refresh system prompt.");
        }
        let userPrompt = USER.tableBaseSetting.refresh_user_message_template;
        ...
        console.log('Raw API Content:', rawContent);

        // 清洗API响应
        let cleanContent = cleanApiResponse(rawContent);
        console.log('Cleaned API Content:', cleanContent);
        ...
        try {
            ...
            console.error('[解析错误] 详细日志：', {
                rawContent: cleanContent,
                errorPosition: parseError.stack,
                previewText: cleanContent.slice(Math.max(0, position - 50), position + 50)
            });
            EDITOR.error(`JSON解析失败：${parseError.message}`);
            return 'error';
        }

        // 去重并确保删除操作顺序 (与 refreshTableActions 逻辑一致)
        let uniqueActions = [];
        const deleteActions = [];
        const nonDeleteActions = [];
        actions.forEach(action => {
        ...
        const uniqueNonDeleteActions = nonDeleteActions.filter((action, index, self) => {
            if (action.action.toLowerCase() === 'insert') {
                const table = DERIVED.any.waitingTable[action.tableIndex];
                if (!table) {
                    console.warn(`表索引 ${action.tableIndex} 无效，跳过插入操作:`, action);
        ...
        if (uniqueActions.length === 0) {
            EDITOR.info("AI未返回任何有效的表格操作。");
            return 'success'; // Or 'no_op' if you want to distinguish
        }

        // 如果不是静默更新，显示操作确认
        if (!silentUpdate) {
            const confirmContentData = uniqueActions.map(action => {
                 const table = DERIVED.any.waitingTable[action.tableIndex];
                 const tableName = table ? table.tableName : `表 ${action.tableIndex}`;
                 const columns = table ? table.columns : [];
                 let content = [];
                 if (action.action === 'insert') {
                    content.push(Object.values(action.data));
                 } else if (action.action === 'update') {
                    if (table && table.content && table.content[action.rowIndex]) {
                         content.push(Object.values(action.data)); // Show the new data
                    }
                 } else if (action.action === 'delete') {
                    if (table && table.content && table.content[action.rowIndex]) {
                        content.push(table.content[action.rowIndex]); // Show data to be deleted
                    }
                 }
                 return { tableName, columns, content, action: action.action, rowIndex: action.rowIndex };
            });
        ...
        // 执行操作
        uniqueActions.forEach(action => {
            const tableToUpdate = DERIVED.any.waitingTable[action.tableIndex];
            if (!tableToUpdate) {
                console.error(`执行操作失败: 表索引 ${action.tableIndex} 无效。操作:`, action);
                return; // Skip this action
            }
            switch (action.action.toLowerCase()) {
                case 'update':
                    try {
                        // Ensure content exists and rowIndex is valid
                        if (!tableToUpdate.content || action.rowIndex < 0 || action.rowIndex >= tableToUpdate.content.length) {
                             console.warn(`Skipped update: table ${action.tableIndex} row ${action.rowIndex} 无效或不存在`);
                             break;
                        }
                        const targetRow = tableToUpdate.content[action.rowIndex];
                        if (!targetRow || (Array.isArray(targetRow) && targetRow.length > 0 && !targetRow[0]?.trim()) ) { // Check first cell if it's an array
                            console.log(`Skipped update: table ${action.tableIndex} row ${action.rowIndex} 第一列为空或行无效`);
                            break;
                        }
                        updateRow(action.tableIndex, action.rowIndex, action.data); // updateRow uses DERIVED.any.waitingTable
                        console.log(`Updated: table ${action.tableIndex}, row ${action.rowIndex}`, DERIVED.any.waitingTable[action.tableIndex].content[action.rowIndex]);
                    } catch (error) {
                        console.error(`Update操作失败: ${error.message}`, action);
                    }
                    break;
                case 'insert':
                    try {
                        ...
                        // const isDataComplete = requiredColumns.every((_, index) => action.data.hasOwnProperty(index.toString()));
                        // if (!isDataComplete) {
                        //     console.error(`插入失败：表 ${action.tableIndex} 缺少必填列数据`, action.data, requiredColumns);
                        //     break;
                        // }
                        insertRow(action.tableIndex, action.data); // insertRow uses DERIVED.any.waitingTable
                        console.log(`Inserted into table ${action.tableIndex}`, action.data);
                    } catch (error) {
                         console.error(`Insert操作失败: ${error.message}`, action);
                    }
                    break;
                case 'delete':
                    try {
                        if (action.tableIndex === 0 || !USER.tableBaseSetting.bool_ignore_del) {
                             // Ensure content exists and rowIndex is valid
                            if (!tableToUpdate.content || action.rowIndex < 0 || action.rowIndex >= tableToUpdate.content.length) {
                                console.warn(`Skipped delete: table ${action.tableIndex} row ${action.rowIndex} 无效或不存在`);
                                break;
                            }
                            const deletedRow = DERIVED.any.waitingTable[action.tableIndex].content[action.rowIndex];
                            deleteRow(action.tableIndex, action.rowIndex); // deleteRow uses DERIVED.any.waitingTable
                            console.log(`Deleted: table ${action.tableIndex}, row ${action.rowIndex}`, deletedRow);
                        } else {
                            console.log(`Ignored delete: table ${action.tableIndex}, row ${action.rowIndex}`);
                        }
                    } catch (error) {
                        console.error(`Delete操作失败: ${error.message}`, action);
                    }
                    break;
            }
        });

        if (USER.tableBaseSetting.bool_ignore_del && uniqueActions.some(a => a.action.toLowerCase() === 'delete' && a.tableIndex !== 0)) {
